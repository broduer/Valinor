...
public class Entity extends Renderable {
    ...
    public void refreshEntityFaceDirection() {
        if (cease_movement == Client.game_tick || animation == -1
                ...
                + 1 > Sequence.cache[animation]
                        .duration(current_animation_frame)) {
    ...
    public void updateAnimation() {
        try {
            if (queued_animation_id > 13798) {
                queued_animation_id = -1;
            }
            dynamic = false;
            if (queued_animation_id != -1) {
                Sequence animation = Sequence.cache[queued_animation_id];
                queued_animation_duration++;
                if (queued_animation_frame < animation.frameCount && queued_animation_duration > animation.duration(queued_animation_frame)) {
                    queued_animation_duration = 1;
                    queued_animation_frame++;
                    next_idle_frame++;
                }

                next_idle_frame = queued_animation_frame + 1;
                if (next_idle_frame >= animation.frameCount) {
                    if (next_idle_frame >= animation.frameCount)
                        next_idle_frame = 0;
                }

                if (queued_animation_frame >= animation.frameCount) {
                    queued_animation_duration = 1;
                    queued_animation_frame = 0;
                }
            }
            if (graphic_id != -1 && Client.game_tick >= graphic_cycle) {
                if (current_animation_id < 0)
                    current_animation_id = 0;

                Sequence animation_1 = SpotAnimation.cache[graphic_id].seq;

                for (current_animation_time_remaining++; current_animation_id < animation_1.frameCount && current_animation_time_remaining > animation_1.duration(current_animation_id); current_animation_id++)
                    current_animation_time_remaining -= animation_1.duration(current_animation_id);

                if (current_animation_id >= animation_1.frameCount && (current_animation_id < 0 || current_animation_id >= animation_1.frameCount))
                    graphic_id = -1;

                if (Client.singleton.setting.enableTweening) {
                    next_animation_frame = queued_animation_frame + 1;
                }

                if (next_graphic_frame >= animation_1.frameCount) {
                    next_graphic_frame = -1;
                }
            }
            if (animation != -1 && animation_delay <= 1) {
                Sequence animation_2 = Sequence.cache[animation];
                if (animation_2.animatingPrecedence == 1 && remaining_steps > 0 && initiate_movement <= Client.game_tick && cease_movement < Client.game_tick) {
                    animation_delay = 1;
                    return;
                }
            }
            if (animation != -1 && animation_delay == 0) {
                Sequence animation_3 = Sequence.cache[animation];
                for (current_animation_duration++; current_animation_frame < animation_3.frameCount && current_animation_duration > animation_3.duration(current_animation_frame); current_animation_frame++)
                    current_animation_duration -= animation_3.duration(current_animation_frame);

                if (current_animation_frame >= animation_3.frameCount) {
                    current_animation_frame -= animation_3.loopOffset;
                    animation_loops++;
                    if (animation_loops >= animation_3.maximumLoops)
                        animation = -1;
                    if (current_animation_frame < 0 || current_animation_frame >= animation_3.frameCount)
                        animation = -1;
                }
                if (Client.singleton.setting.enableTweening) {
                    next_animation_frame = current_animation_frame + 1;
                }

                if (next_animation_frame >= animation_3.frameCount) {
                    next_animation_frame = -1;
                }
                dynamic = animation_3.stretches;
            }
