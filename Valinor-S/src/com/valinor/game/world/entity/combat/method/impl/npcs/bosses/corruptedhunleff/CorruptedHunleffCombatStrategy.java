    ...
    private void rangeAttack(Mob mob, Mob target) {
        ...
        World.getWorld().getPlayers().forEach(p -> {
            if(p != null && target.tile().inSqRadius(p.tile(),12)) {
                var tileDist = mob.tile().transform(1, 1, 0).distance(p.tile());
                var delay = Math.max(1, (50 + (tileDist * 12)) / 30);
                Projectile projectile = new Projectile(mob, p, 1705, 35, 20 * tileDist, 45, 30, 0);
    ...
    private void magicAttack(Mob mob, Mob target) {
        ...
        World.getWorld().getPlayers().forEach(p -> {
            if(p != null && target.tile().inSqRadius(p.tile(),12)) {
                ...
                if (World.getWorld().rollDie(4, 1)) {
                    prayerDisableAttack(mob, p);
                } else {
                    var tileDist = mob.tile().transform(1, 1, 0).distance(p.tile());
                    var delay = Math.max(1, (50 + (tileDist * 12)) / 30);
                    Projectile projectile = new Projectile(mob, p, 1713, 35, 20 * tileDist, 45, 30, 0);
                    projectile.sendProjectile();
                    p.hit(mob, CombatFactory.calcDamageFromType(mob, p, CombatType.MAGIC), delay, CombatType.MAGIC).checkAccuracy().submit();
                    p.delayedGraphics(1709, 100, delay);
                }
            }
        });
