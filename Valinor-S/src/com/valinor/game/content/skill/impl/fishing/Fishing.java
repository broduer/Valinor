...
public class Fishing {
    ...
    private static void unlockHeron(Player player) {
        if (!PetAI.hasUnlocked(player, Pet.HERON)) {
            ...
            World.getWorld().sendWorldMessage("<img=452><shad=0>" + Color.RED.wrap(player.getUsername()) + " has unlocked the pet: <col="+Color.HOTPINK.getColorValue()+">" + new Item(Pet.HERON.item).name()+ "</col>.");
        } else {
