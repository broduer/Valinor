 ...
 */
public class Vannaka extends PacketInteraction {
    ...
    private static void assignTask(Player player) {
        ...
        player.putAttrib(AttributeKey.SLAYER_MASTER, Slayer.VANNAKA_ID);
    ...
    public static void giveTask(Player player) {
        ...
        if (numleft > 0) {
            DialogueManager.npcChat(player, Expression.H, VANNAKA, "You're still hunting " + Slayer.taskName(player.slayerTaskId()) + "; you have " + numleft + " to go. Come", "back when you've finished your task.");
            return;
        }
        ...
        player.getDialogueManager().start(new Dialogue() {
            ...
            protected void start(Object... parameters) {
                send(DialogueType.NPC_STATEMENT, VANNAKA, Expression.ANXIOUS, "Excellent, you're doing great. Your new task is to kill", ""+num+" "+Slayer.taskName(task.uid)+".");
                setPhase(0);
            }
            ...
            protected void next() {
                if(isPhase(0)) {
                    ...
                    send(DialogueType.NPC_STATEMENT, VANNAKA, Expression.ANXIOUS, Slayer.tipFor(task));
                    setPhase(3);
                } else if(isPhase(3)) {
